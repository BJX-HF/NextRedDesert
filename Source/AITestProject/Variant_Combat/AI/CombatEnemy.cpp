...
void ACombatEnemy::InitializeAbilitySystem()
{
	...
	AbilitySystemComponent->GetGameplayAttributeValueChangeDelegate(UAITestAttributeSet::GetHealthAttribute()).AddUObject(this, &ACombatEnemy::HandleHealthChanged);
	AbilitySystemComponent->SetNumericAttributeBase(UAITestAttributeSet::GetMaxHealthAttribute(), MaxHP);
	AbilitySystemComponent->SetNumericAttributeBase(UAITestAttributeSet::GetHealthAttribute(), MaxHP);
...
void ACombatEnemy::GrantStartupAbilities()
{
...
void ACombatEnemy::HandleHealthChanged(const FOnAttributeChangeData& ChangeData)
{
	CurrentHP = ChangeData.NewValue;

	if (LifeBarWidget && AttributeSet && AttributeSet->GetMaxHealth() > 0.0f)
	{
		LifeBarWidget->SetLifePercentage(CurrentHP / AttributeSet->GetMaxHealth());
	}
}
...
{
	if (!AbilitySystemComponent || !AttributeSet || Healing <= 0.0f || CurrentHP <= 0.0f)
	{
		return;
	}

	AbilitySystemComponent->ApplyModToAttribute(UAITestAttributeSet::GetHealthAttribute(), EGameplayModOp::Additive, Healing);
}
...
float ACombatEnemy::TakeDamage(float Damage, struct FDamageEvent const& DamageEvent, AController* EventInstigator, AActor* DamageCauser)
{
	...
	if (!AbilitySystemComponent || !AttributeSet || CurrentHP <= 0.0f || Damage <= 0.0f)
	{
		return 0.0f;
	}

	const float PreviousHealth = CurrentHP;
	AbilitySystemComponent->ApplyModToAttribute(UAITestAttributeSet::GetHealthAttribute(), EGameplayModOp::Additive, -Damage);
	...
	return PreviousHealth - CurrentHP;
}
...
void ACombatEnemy::BeginPlay()
{
	...
	// initialize GAS and sync the initial health state
